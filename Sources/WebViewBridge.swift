...
class WebViewBridge {
    ...
    func handle(_ message: NSDictionary) {
        ...
        if messageType == "newMessage" {
            ...
            let chatioMessage = ChatioMessage(id: messageId, text: text, date: Date(timeIntervalSince1970: timestamp), authorName: authorName, rawMessage: message)
            
            delegate?.received(message: chatioMessage)
    ...
    func postFocusEvent() {
        post(message: "ChatioWidget_emitFocus();")
    }
    
    func postBlurEvent() {
        post(message: "ChatioWidget_emitBlur();")
    }
